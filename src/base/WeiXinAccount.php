...
class WeiXinAccount extends WeAccount {
    ...
    public function local_decryptMsg($postData) {
        ...
        $iv = substr($key, 0, 16);
        $decrypted = openssl_decrypt($packet['Encrypt'], 'AES-256-CBC', $key, 0, $iv);
    ...
    public function encryptMsg($text) {
        ...
        */
        ...
        $encrypt_msg = openssl_encrypt($text, 'AES-256-CBC', $key, 0, $iv);
    ...
    public function decryptMsg($postData) {
        ...
        $istrue = $this->checkSignature(trim($packet['encrypt']));
        if(!$istrue) {
            return error(-1, "微信公众平台返回接口错误. \n错误代码为: 40001 \n,错误描述为: " . $this->encrypt_error_code('40001'));
        }
    ...
    public function getOauthInfo($code = '') {
        ...
        if (empty($code)) {
            $unisetting = uni_setting_load();//$sitepath .
            $url = (!empty($unisetting['oauth']['host']) ? ($unisetting['oauth']['host'] .  '/') : $_W['siteroot'] . 'app/') . "index.php?{$_SERVER['QUERY_STRING']}";
