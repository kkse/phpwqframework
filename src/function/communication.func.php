...
function ihttp_request($url, $post = '', $extra = array(), $timeout = 60) {
	...
	if (function_exists('curl_init') && function_exists('curl_exec')) {
        ...
        if (\Overtrue\Wechat\Utils\Http::$PROXY) {
            $PROXY = \Overtrue\Wechat\Utils\Http::$PROXY;
            curl_setopt($ch, CURLOPT_PROXY, $PROXY['HOST']);
            empty($PROXY['AUTH']) or curl_setopt($ch, CURLOPT_PROXYAUTH, $PROXY['AUTH']);
            empty($PROXY['PORT']) or curl_setopt($ch, CURLOPT_PROXYPORT, $PROXY['PORT']);
            empty($PROXY['TYPE']) or curl_setopt($ch, CURLOPT_PROXYTYPE, $PROXY['TYPE']);
            empty($PROXY['USERPWD']) or curl_setopt($ch, CURLOPT_PROXYUSERPWD, $PROXY['USERPWD']);
            empty($PROXY['TUNNEL']) or curl_setopt($ch, CURLOPT_HTTPPROXYTUNNEL, true);
        }
