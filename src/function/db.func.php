...
function db_table_schema($tablename = '') {
    $result = pdo_fetch("SHOW TABLE STATUS LIKE '" . tablename($tablename, false) . "'");
    ...
    $result = pdo_fetchall("SHOW FULL COLUMNS FROM " . tablename($tablename));
    ...
    $result = pdo_fetchall("SHOW INDEX FROM " . tablename($tablename));
	foreach($result as $value) {
...
function db_table_serialize( $dbname) {
	...
	foreach ($tables as $value) {
		$structs[] = db_table_schema(substr($value['Tables_in_' . $dbname], strpos($value['Tables_in_' . $dbname], '_') + 1));
	}
...
function db_table_create_sql($schema) {
	...
	$schema['tablename'] = str_replace('ims_', config('DB_TABLEPRE'), $schema['tablename']);
