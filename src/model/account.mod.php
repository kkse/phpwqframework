...
use dtwq\system\ModuleManage as ExtModule;
...
function uni_create_permission($uid, $type = 1) {
	...
	$uniacidnum = count($list);
		...
		$subaccountnum = 0;
		if (!empty($uniacids)) {
			$subaccountnum = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('account') . ' WHERE uniacid IN (' . implode(',', $uniacids) . ')');
		}
...
function uni_permission($uid = 0, $uniacid = 0) {
	...
	if ($uid == $_W['uid'] && $_W['isfounder']) {
		return 'founder';
	}

    $role = pdo_getcolumn('uni_account_users', ['uniacid'=>$uniacid, 'uid'=>$uid], 'role');
...
function uni_modules($enabledOnly = true) {
	...
	$extend = pdo_getall('uni_account_group', array('uniacid' => $_W['uniacid']), [], [], 'groupid');
	if (!empty($extend)) {
		$groupid = '-2';
	}
...
function _uni_user_permission_check($permission_name, $is_html = true, $action = '') {
    global $_W, $_GPC;
    ...
    $eid = intval($_GPC['eid']);
    if($action == 'reply') {
        $system_modules = ExtModule::system_modules();
...
function get_system_permission($account_type = '', $idstr = '')
{
    global $_W, $_GPC;
    static $cache_data = array();
    $account_type or $account_type = $_W['user']['idtype'];;
    $idstr or $idstr = $_W['user']['idstr'];
  
    if (isset($cache_data[$account_type][$idstr])) {
        return $cache_data[$account_type][$idstr];
    }
	
	if($account_type != 'SYS'){
		
		$cache_data[$account_type][$idstr] = array();
		
		$where = ' WHERE idtype = "'.$account_type.'" AND (idstr = "SYS" OR idstr = "'.$idstr.'")';
		
		$system_permission = pdo_fetchall('SELECT mnt.permission_name FROM ' .tablename('modules_node_dt'). ' AS mnt '.$where );
			
		if (!empty($system_permission)) {
			$cache_data[$account_type][$idstr] = array_column($system_permission,'permission_name');
		}
    }
...
{
    return true;
    //return _uni_user_permission_check($permission_name, $is_html, $action);
}
...
function uni_update_week_stat() {
	...
	$week_stat_fans = pdo_getall('stat_fans', array('date' => ['in', $seven_days], 'uniacid' => $_W['uniacid']), [],  [],'date');
	...
	foreach($seven_days as $sevens) {
		if($_W['account']['level'] == ACCOUNT_SUBSCRIPTION_VERIFY || $_W['account']['level'] == ACCOUNT_SERVICE_VERIFY) {
			...
			if(is_error($weixin_stat) || empty($weixin_stat)) {
				...
				$update_stat = array(
					'uniacid' => $_W['uniacid'],
					'new' => intval($weixin_stat[$sevens]['new']),
					'cancel' => intval($weixin_stat[$sevens]['cancel']),
					'cumulate' => intval($weixin_stat[$sevens]['cumulate']),
					'date' => $sevens,
				);
			}
		} else {
			$update_stat = array();
			$update_stat['cumulate'] = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('mc_mapping_fans') . " WHERE acid = :acid AND uniacid = :uniacid AND follow = :follow AND followtime < :endtime", array(':acid' => $_W['acid'], ':uniacid' => $_W['uniacid'], ':endtime' => strtotime($sevens)+86400, ':follow' => 1));
			$update_stat['date'] = $sevens;
			$update_stat['new'] = intval($week_stat_fans[$sevens]['new']);
			$update_stat['cancel'] = intval($week_stat_fans[$sevens]['cancel']);
...
function uni_setmeal($uniacid = 0) {
    ...
    $owneruid = pdo_getcolumn('uni_account_users', ['uniacid'=>$uniacid, 'role'=>'owner'], 'uid');
	if(empty($owneruid)) {
		$user = array(
			'uid' => -1,
			'username' => '创始人',
...
function uni_is_multi_acid($uniacid = 0) {
	...
	$nums = intval($nums);
	if(!$nums) {
		$nums = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('account_wechats') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
...
function account_delete($acid) {
	...
	if ($account) {
		...
		if (!empty($subaccount)) {
			...
			'coupon_record', 'cover_reply', 'mc_card','mc_chats_record','mc_credits_recharge','mc_credits_record',
		...
		if (!empty($tables)) {
			foreach ($tables as $table) {
				$tablename = str_replace(config('DB_TABLEPRE'), '', $table);
...
function new_user_menu_frame_key()
{
	global $_W;
	$idtype = $_W['user']['idtype'];
	$idstr = $_W['user']['idstr'];
    return 'new_system_frame_'.$idtype.'_'.$idstr;
    
	return 'new_system_frame_'.$_W['uid'];
}

function user_menu_frame_key()
{
	global $_W;
	$idtype = $_W['user']['idtype'];
	$idstr = $_W['user']['idstr'];
    return 'system_frame_'.$idtype.'_'.$idstr;
    
	return 'system_frame_'.$_W['uid'];
}

function user_menu_fetchall($where, $fields = '*', $order = '')
{
	global $_W;
    $table = 'core_menu';
	$idtype = $_W['user']['idtype'];
	...
	$add_where = " AND idtype='{$idtype}' AND (idstr in ('SYS','TOT') OR idstr='{$idstr}') ";

	
	$sql = sprintf('SELECT %s FROM %s WHERE %s %s %s', $fields, tablename($table), $where, $add_where, $order?('ORDER BY '.$order):'');
	
	return pdo_fetchall($sql);
}

function core_menu_frame_key()
{
	global $_W;
	$idtype = $_W['user']['idtype'];
	$idstr = $_W['user']['idstr'];
...
function core_menu_fetchall($where, $fields = '*', $order = '')
{
	global $_W;
	$idtype = $_W['user']['idtype'];
	$idstr = $_W['user']['idstr'];
	...
	$sql = sprintf('SELECT %s FROM %s WHERE %s %s %s', $fields, tablename('core_menu'), $where, $add_where, $order?('ORDER BY '.$order):'');
	return pdo_fetchall($sql);
}
...
{
	global $_W;
	return $_W['user']['idtype'] == 'TOT' && $_W['user']['idstr'] == 'TOT';
}
