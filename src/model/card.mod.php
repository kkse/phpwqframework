...
function card_notice_stat() {
	...
	$total = 0;
	$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('mc_card_notices_unread') . ' WHERE uniacid = :uniacid AND uid = :uid AND is_new = 1', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']));
	return $total;
}
