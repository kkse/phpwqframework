...
function activity_coupon_available($uid, $pindex = 1, $psize = 10) {
	...
	foreach ($data as $da) {
		...
		$pcount = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('activity_coupon_record') . " WHERE `uid` = :uid AND `couponid` = :couponid", array(':uid' => $uid, ':couponid' => $da['couponid']));
		if ($pcount < $da['limit']) {
			$id[] = $da['couponid'];
		}
	}
	if (!empty($id)) {
		$idstr = implode(',', $id);
		$total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('activity_coupon') . " WHERE `type` = 1 AND `couponid` IN ({$idstr})");
		if ($psize > 0) {
...
function activity_coupon_owned($uid, $filter = array(), $pindex = 10, $psize = 0) {
	...
	$total = pdo_fetchall("SELECT COUNT(1) AS cototal, r.couponid, r.code, r.status FROM " . tablename('activity_coupon_record') . " AS r LEFT JOIN " . tablename('activity_coupon') . " AS c ON r.couponid = c.couponid WHERE c.type = 1 AND r.uid = :uid " . $condition . ' GROUP BY r.couponid', array(':uid' => $uid));
	$data = pdo_fetchall("SELECT COUNT(1) AS cototal, r.couponid, r.code, r.recid, r.status FROM " . tablename('activity_coupon_record') . " AS r LEFT JOIN " . tablename('activity_coupon') . " AS c ON r.couponid = c.couponid WHERE c.type = 1 AND r.uid = :uid " . $condition . ' GROUP BY r.couponid ORDER BY r.couponid DESC' . $limit_sql, array(':uid' => $uid), 'couponid');
	if(!empty($data)) {
...
function activity_coupon_grant($uid, $couponid, $module = '', $remark = '') {
	...
	$pcount = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('activity_coupon_record') . " WHERE `uid` = :uid AND `couponid` = :couponid", array(':couponid' => $couponid, ':uid' => $uid));
...
function activity_token_available($uid, $pindex = 1, $psize = 0) {
	...
	foreach ($coupons as $coupon) {
		...
		if ($coupon['amount'] > $coupon['dosage']) {
			$sql = 'SELECT COUNT(1) FROM ' . tablename('activity_coupon_record') . ' WHERE `uniacid` = :uniacid AND
	...
	$sql = 'SELECT COUNT(1) FROM ' . tablename('activity_coupon') . ' WHERE `type` = :type AND `couponid` IN (' .
			$couponIds . ')';
	$params = array(':type' => 2);
...
function activity_token_owned($uid, $filter = array(), $pindex = 1, $psize = 10) {
	...
	$total = pdo_fetchall("SELECT COUNT(1) AS cototal, r.couponid, r.code, r.status FROM " . tablename('activity_coupon_record') . " AS r LEFT JOIN " . tablename('activity_coupon') . " AS c ON r.couponid = c.couponid WHERE c.type = 2 AND r.uid = :uid " . $condition . ' GROUP BY r.couponid', array(':uid' => $uid));
...
function activity_goods_grant($uid, $exid){
	...
	$pnum = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('activity_exchange_trades') . ' WHERE uniacid = :uniacid AND uid = :uid AND exid = :exid', array(':uniacid' => $_W['uniacid'], ':uid' => $uid, ':exid' => $exid));
	...
	if ($exchange['pretotal'] > 0) {
		...
		if ($activity_modules) {
			$starttime = strtotime(date('Y-m-d')) - intval($exchange['extra']['period']) * 3600 * 24;
						$pnum = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('activity_modules_record') . ' WHERE mid = :mid AND num = 1 AND createtime > :createtime', array('mid' => $activity_modules['mid'], ':createtime' => $starttime));
			if ($pnum >= $exchange['pretotal']) {
...
function activity_module_card_own($uid, $filter = array('module' => '', 'used' => '', 'type' => '1'), $pindex = 1, $psize = 10) {
	...
	$data = pdo_fetchall('SELECT a.*, b.*,COUNT(1) as num FROM ' . tablename('activity_coupon_record') . ' AS a LEFT JOIN ' . tablename('activity_coupon') . ' AS b ON a.couponid = b.couponid' . $condition . ' ORDER BY b.endtime ASC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, $params);
	return $data;
}