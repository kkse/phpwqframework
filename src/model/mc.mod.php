...
function mc_check($data) {
	...
	if (!empty($data['email'])) {
		...
		$isexist = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('mc_members') . ' WHERE email = :email AND uid != :uid', array( ':email' => $email, ':uid' => $_W['member']['uid']));
		if ($isexist >= 1) {
	...
	if (!empty($data['mobile'])) {
		...
		$isexist = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('mc_members') . ' WHERE mobile = :mobile AND uid != :uid', array(':mobile' => $mobile, ':uid' => $_W['member']['uid']));
		if ($isexist >= 1) {
...
function mc_update($uid, $fields) {
	...
	if (!empty($isexists)) {
		...
		$emailexists = pdo_fetchcolumn("SELECT email FROM " . tablename('mc_members') . " WHERE  email = :email " . $condition, array( ':email' => trim($fields['email'])));
		if ($emailexists) {
			unset($fields['email']);
		}
	}
	if (!empty($fields['mobile'])) {
		$mobilexists = pdo_fetchcolumn("SELECT mobile FROM " . tablename('mc_members') . " WHERE  mobile = :mobile " . $condition, array( ':mobile' => trim($fields['mobile'])));
		if ($mobilexists) {
			unset($fields['mobile']);
		}
	}
...
function mc_oauth_userinfo($must_openid = true, $must_uid = false) {
	global $_W;

	$ret = checkauth($must_openid, $must_uid);

    if ($ret) {
        if ($_W['fans']) {
            $fan = $_W['fans'];
            if ($must_uid) {
                $fan['uid'] = $_W['member']['uid'];
            }
            return $fan;
        } elseif ($must_uid) {
            $fan = [];
            $fan['avatar'] = $fan['headimgurl'] = $_W['member']['avatar'];
            $fan['uid'] = $_W['member']['uid'];
            return $fan;
        } else {
            return [];
	...
	if (!empty($message)) {
		...
		if (checksubmit('submit')) {
			...
			if (in_array('email', $fields)) {
				$emailexists = pdo_fetchcolumn("SELECT email FROM " . tablename('mc_members') . " WHERE  email = :email " . $condition, array(':email' => trim($record['email'])));
				if (!empty($emailexists)) {
					message('抱歉，您填写的手机号已经被使用，请更新。', 'refresh', 'error');
				}
			}
			if (in_array('mobile', $fields)) {
				$mobilexists = pdo_fetchcolumn("SELECT mobile FROM " . tablename('mc_members') . " WHERE  mobile = :mobile " . $condition, array(':mobile' => trim($record['mobile'])));
...
function dt_mc_login($member) {
    global $_W;

    if (is_int($member)) {
        $visitor = \Org\Member\Visitor::getInstance();
        $visitor->setUid($member, true);
        $member = $visitor->getMember();
    }

    if ($member instanceof \Org\Base\Member) {
        $_W['member'] = $member->toArray();
        $_W['member']['groupname'] = $_W['uniaccount']['groups'][$_W['member']['groupid']]['title'];
        mc_group_update();
        return true;
    }
    return false;
...
function mc_group_update($uid = 0) {
	...
	if($uid <= 0) {
		...
		$user = pdo_fetch('SELECT uid, realname, credit1, credit6, groupid FROM ' . tablename('mc_members') . ' WHERE uid = :uid', array( ':uid' => $uid));
	...
	if($groupid > 0 && $groupid != $user['groupid']) {
		pdo_update('mc_members', array('groupid' => $groupid), array('uid' => $uid));
